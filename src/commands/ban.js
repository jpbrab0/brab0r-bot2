    ...
    if (targetBan[1] == undefined) {
      ...
      switch (msgRandom) {
        ...
        case 2:
          client.say(
            target,
            `${context.username} baniu ${targetBan[1]} por não ter feito bootcamp de 2 dias de js >( `
          );
          break;
        case 3:
          client.say(
      ...
      if (randomBan > 300 && randomBan < 2000) {
        ...
        client.say(target, `${targetBan[1]} deu azar e foi banido(a)...`);
      } else if (randomBan > 2500 && randomBan < 6200) {
        client.say(target, `/timeout ${context.username} 30`);
        client.say(target, `${context.username} deu azar e foi banido(a)...`);
      } else {
